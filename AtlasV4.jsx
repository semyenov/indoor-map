...
const filterSpaces = (q) => {
  ...
  return SPACES.filter(s =>
    ...
    s.dept.toLowerCase().includes(lq) ||
    (KIND_L[s.kind] || "").toLowerCase().includes(lq)
  );
};
...
function SpaceCard({ space, onClick, selected, compact }) {
  ...
  return (
    ...
    >
      <div style={{ ...S.cardAccent, background: st?.c }} />
      <div style={S.cardTop}>
...
export default function AtlasV4() {
  ...
  return (
    <div style={S.shell}>
      ...
      {/* ═══ MAP BG ═══ */}
      <div style={S.mapBg}>
        <svg width="100%" height="100%" viewBox="0 0 1200 700" fill="none" preserveAspectRatio="xMidYMid slice">
          {/* Fine grid */}
          {Array.from({ length: 21 }).map((_, i) => (
            <line key={`vf${i}`} x1={i * 60} y1="0" x2={i * 60} y2="700" stroke="currentColor" strokeWidth="0.3" opacity="0.04" />
          ))}
          {Array.from({ length: 13 }).map((_, i) => (
            <line key={`hf${i}`} x1="0" y1={i * 60} x2="1200" y2={i * 60} stroke="currentColor" strokeWidth="0.3" opacity="0.04" />
          ))}
          {/* Major grid */}
          {[200, 400, 600, 800, 1000].map((x) => (
            <line key={`vm${x}`} x1={x} y1="0" x2={x} y2="700" stroke="currentColor" strokeWidth="0.5" opacity="0.07" />
          ))}
          {[175, 350, 525].map((y) => (
            <line key={`hm${y}`} x1="0" y1={y} x2="1200" y2={y} stroke="currentColor" strokeWidth="0.5" opacity="0.07" />
          ))}
          {/* Building boundary */}
          <rect x="80" y="60" width="1040" height="580" rx="4" stroke="currentColor" strokeWidth="1" opacity="0.14" />
          {/* Upper zone */}
          <rect x="80"  y="60"  width="480" height="260" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          <rect x="590" y="60"  width="250" height="260" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          <rect x="870" y="60"  width="250" height="260" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          {/* Lower zone */}
          <rect x="80"  y="340" width="260" height="300" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          <rect x="370" y="340" width="180" height="300" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          <rect x="580" y="340" width="180" height="300" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          <rect x="790" y="340" width="150" height="300" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          <rect x="970" y="340" width="150" height="300" rx="2" stroke="currentColor" strokeWidth="0.6" opacity="0.1" />
          {/* Center marker */}
          <circle cx="600" cy="350" r="4" stroke="currentColor" strokeWidth="0.6" opacity="0.08" />
          <line x1="582" y1="350" x2="618" y2="350" stroke="currentColor" strokeWidth="0.4" opacity="0.07" />
          <line x1="600" y1="332" x2="600" y2="368" stroke="currentColor" strokeWidth="0.4" opacity="0.07" />
        </svg>
      </div>
      ...
      <header style={S.topBar}>
        <div style={S.topLeft}>
          ...
          <button style={S.searchBtn} className="hud-glass hud-hover" onClick={openBrowse}>
            <Ic.Search s={14} />
            <span style={{ color: T.muted, fontWeight: 400 }}>Browse spaces & people…</span>
            <kbd style={S.kbd}>/</kbd>
          </button>
        </div>
        ...
        <div style={S.topCenter}>
          <div style={S.viewModes} className="hud-glass">
            {[
              { id: "plan",    ic: <Ic.Floor />,   l: "Plan"    },
              { id: "explore", ic: <Ic.Compass />, l: "Explore" },
              { id: "theatre", ic: <Ic.Eye />,     l: "Theatre" },
            ].map(m => (
              <button
                key={m.id}
                style={{ ...S.vmBtn, ...(viewMode === m.id ? S.vmActive : {}) }}
                className="hud-btn"
        ...
        <div style={S.topRight}>
          <button style={S.opsBtn} className="hud-glass hud-hover" onClick={() => setOpsOpen(!opsOpen)}>
            ...
            <span style={S.opsBadge}>
              <span style={{ ...S.liveDot, background: ST.available.c }} />
              {cts.available} free
            </span>
          </button>
          <div style={S.syncChip} className="hud-glass">
            <span style={{ ...S.liveDot, background: '#34d399' }} />
      ...
      <div style={S.bottomLeft}>
        <div style={S.floorPicker} className="hud-glass">
          {["L1", "L2"].map(l => (
            <button
              key={l}
              style={{ ...S.floorBtn, ...(level === l ? S.floorBtnActive : {}) }}
              className="hud-btn"
              onClick={() => setLevel(l)}
            >
              {l}
          ...
          <div style={S.browsePanel} className="hud-glass-heavy oa-fade" onClick={e => e.stopPropagation()}>
            <div style={S.bpHeader}>
              <div style={S.bpSearchRow}>
                ...
                />
                {browseQ && (
                  <button style={S.iconBtn} className="hud-btn" onClick={() => setBrowseQ("")}>
                    <Ic.X s={12} />
                  </button>
                )}
                <div style={S.bpDivider} />
                <button style={S.iconBtn} className="hud-btn" onClick={() => setBrowseOpen(false)}>
                  <Ic.X />
                </button>
              </div>
              <div style={S.bpToolbar}>
                <div style={S.bpGroupRow}>
                  ...
                  {GROUP_OPTIONS.map(g => (
                    <button
                      key={g.key}
                      style={{ ...S.pill, ...(browseGroup === g.key ? S.pillActive : {}) }}
                      className="hud-btn"
                      onClick={() => setBrowseGroup(g.key)}
                    >
                      {g.label}
                    </button>
                  ))}
                ...
                <span style={S.bpCount}>
                  {browseSpaces.length} spaces{browsePeople.length > 0 ? ` · ${browsePeople.length} people` : ''}
                </span>
              </div>
            </div>
            ...
            <div style={S.rpHeader}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <label style={S.checkRow}>
                  <div
                    style={{ ...S.checkBox, ...(accessible ? S.checkBoxOn : {}) }}
                    onClick={e => { e.preventDefault(); setAccessible(!accessible); }}
                  >
                    {accessible && <Ic.Check />}
                  </div>
            ...
            <div style={S.rpColumns}>
              ...
              <div style={S.rpCol}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {routeFrom ? (
                      <div style={S.rpSelected}>
                        ...
                        <button style={S.rpClearBtn} className="hud-btn" onClick={() => setRouteFrom(null)}>
                          <Ic.X s={10} />
                        </button>
                      </div>
                    ) : (
                ...
                <div style={S.rpColSearch}>
                  ...
                  {routeFromQ && (
                    <button style={S.iconBtn} className="hud-btn" onClick={() => setRouteFromQ("")}>
                      <Ic.X s={10} />
                    </button>
                  )}
                </div>
                <div style={S.rpColToolbar}>
                  <span style={{ fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: '.05em' }}>GROUP</span>
                  {GROUP_OPTIONS.slice(0, 3).map(g => (
                    <button
                      key={g.key}
                      style={{ ...S.pillSm, ...(routeFromGroup === g.key ? S.pillSmActive : {}) }}
                      className="hud-btn"
                      onClick={() => setRouteFromGroup(g.key)}
                    >
                      {g.label}
                    </button>
                  ))}
              ...
              <div style={S.rpSwapCol}>
                <button
                  style={S.rpSwapBtn}
                  className="hud-btn"
                  onClick={() => { const t = routeFrom; setRouteFrom(routeTo); setRouteTo(t); }}
                >
                  <Ic.Swap />
                </button>
              </div>
              ...
              <div style={{ ...S.rpCol, borderRight: 'none' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {routeTo ? (
                      <div style={S.rpSelected}>
                        ...
                        <button style={S.rpClearBtn} className="hud-btn" onClick={() => setRouteTo(null)}>
                          <Ic.X s={10} />
                        </button>
                      </div>
                    ) : (
                ...
                <div style={S.rpColSearch}>
                  ...
                  {routeToQ && (
                    <button style={S.iconBtn} className="hud-btn" onClick={() => setRouteToQ("")}>
                      <Ic.X s={10} />
                    </button>
                  )}
                </div>
                <div style={S.rpColToolbar}>
                  ...
                  {GROUP_OPTIONS.slice(0, 3).map(g => (
                    <button
                      key={g.key}
                      style={{ ...S.pillSm, ...(routeToGroup === g.key ? S.pillSmActive : {}) }}
                      className="hud-btn"
                      onClick={() => setRouteToGroup(g.key)}
                    >
                      {g.label}
                    </button>
                  ))}
                ...
                <div style={S.rrHero}>
                  ...
                  <div style={S.rrStats}>
                    {[{ v: "26", l: "Nodes" }, { v: "25", l: "Legs" }, { v: "2", l: "Levels" }, { v: "elev", l: "Via", ico: true }].map(s => (
                      <div key={s.l} style={S.rrStat}>
                        <span style={S.rrStatV}>{s.ico ? <Ic.Elev /> : s.v}</span>
                        <span style={S.rrStatL}>{s.l}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div style={S.rrDirections}>
                  {[
                    ...
                    <div key={i} style={S.rrStep}>
                      <div style={{ ...S.rrStepN, ...(i === 4 ? { background: T.accent, color: '#080c14', borderColor: T.accent } : {}) }}>
                        {i === 4 ? <Ic.Check /> : i + 1}
                      </div>
      ...
      {selected && !browseOpen && !routeOpen && (
        <div style={S.detailFloat} className="hud-glass-heavy oa-slide-left">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: T.accent }}>
              {KIND_L[selected.kind] || selected.kind}
            </span>
            <button style={S.iconBtn} className="hud-btn" onClick={() => setSelected(null)}><Ic.X /></button>
          </div>

          <div>
            <h2 style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-.03em', margin: '0 0 8px', lineHeight: 1.2 }}>{selected.name}</h2>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ ...S.statusPill, color: ST[selected.status]?.c, background: ST[selected.status]?.bg }}>
                <Ic.Pulse /> {ST[selected.status]?.label}
              </span>
              <span style={{ fontSize: 11, fontWeight: 700, fontFamily: MONO, color: T.accent, background: T.accentBg, padding: '3px 8px', borderRadius: 6 }}>
                {selected.level}
              </span>
          ...
          {selected._person && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: T.accentBg, border: `1px solid ${T.accentBorder}`, borderRadius: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{selected._person.name}</div>
                <div style={{ fontSize: 11, color: T.muted }}>Desk {selected._person.desk}</div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: T.border, borderRadius: 12, overflow: 'hidden' }}>
            {[["Department", selected.dept], ["Level", selected.level], ["ID", selected.id]].map(([l, v]) => (
              <div key={l} style={{ display: 'flex', flexDirection: 'column', gap: 3, padding: '10px 12px', background: 'rgba(10,14,24,.6)' }}>
                <span style={{ fontSize: 9, fontWeight: 700, color: T.muted, textTransform: 'uppercase', letterSpacing: '.06em' }}>{l}</span>
                <span style={{ fontSize: 12, fontWeight: 600, ...(l === "ID" ? { fontFamily: MONO, fontSize: 10, color: T.sec } : {}) }}>{v}</span>
              </div>
            ))}
          </div>
          ...
          {equipment.length > 0 && (
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
              {equipment.map(e => (
                <span key={e} style={{ padding: '4px 10px', fontSize: 11, fontWeight: 500, color: T.sec, background: 'rgba(255,255,255,.04)', borderRadius: 6, border: `1px solid ${T.border}` }}>
                  {e}
                </span>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <button
              style={S.accentBtn}
              className="hud-accent"
              onClick={() => { const lob = SPACES[0]; setSelected(null); openRouteBuilder(lob, selected); }}
            >
            ...
            <button
              style={S.ghostBtn}
              className="hud-btn"
              onClick={() => { setSelected(null); openRouteBuilder(selected); }}
            >
        ...
        <div style={S.opsPanel} className="hud-glass-heavy oa-fade">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)' }}>
            {Object.entries(ST).map(([k, cfg], i) => (
              ...
              >
                <span style={{ fontSize: 22, fontWeight: 800, fontFamily: MONO, lineHeight: 1, color: cfg.c }}>{cts[k]}</span>
                <span style={{ fontSize: 9, fontWeight: 600, color: T.muted, textTransform: 'uppercase', letterSpacing: '.06em' }}>{cfg.label}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ overflowY: 'auto', padding: 6, maxHeight: 300 }}>
            {SPACES.filter(s => s.cap > 0).map(s => {
              ...
              return (
                <button
                  key={s.id}
                  style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', background: 'none', border: 'none', borderRadius: 10, fontFamily: FONT, color: T.text, textAlign: 'left', cursor: 'pointer' }}
                  className="hud-card"
                  onClick={() => { selectFromBrowse(s); setOpsOpen(false); }}
                >
                  <span style={{ ...S.statusDot, background: cfg?.c, width: 8, height: 8 }} />
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <span style={{ fontSize: 13, fontWeight: 600 }}>{s.name}</span>
                    <span style={{ fontSize: 11, color: T.muted }}>{s.level} · {s.cap} seats</span>
                  </div>
...
const T = {
  bg:            "#080c14",
  glass:         "rgba(14,20,36,.76)",
  glassH:        "rgba(10,14,28,.94)",
  border:        "rgba(255,255,255,.07)",
  borderH:       "rgba(255,255,255,.13)",
  text:          "#e6e9ee",
  sec:           "rgba(210,220,235,.50)",
  muted:         "rgba(160,175,200,.28)",
  accent:        "#38bdf8",
  accentBg:      "rgba(56,189,248,.09)",
  accentBorder:  "rgba(56,189,248,.24)",
};
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes oa-slide-left{from{opacity:0;transform:translateX(12px)}to{opacity:1;transform:translateX(0)}}
  @keyframes oa-pulse{0%,100%{opacity:1}50%{opacity:.3}}
  .oa-fade{animation:oa-fade .16s ease-out}
  .oa-slide-up{animation:oa-slide-up .2s ease-out}
  .oa-slide-left{animation:oa-slide-left .18s ease-out}
  ...
  .hud-hover{cursor:pointer;transition:background .14s ease,border-color .14s ease}
  .hud-hover:hover{background:${T.glassH}!important;border-color:${T.borderH}!important}
  .hud-btn{cursor:pointer;transition:background .1s ease,color .1s ease}
  .hud-btn:hover{background:rgba(255,255,255,.07)!important}
  .hud-accent{cursor:pointer;transition:background .15s ease,box-shadow .15s ease,transform .08s ease}
  .hud-accent:hover{background:#0ea5e9!important;box-shadow:0 6px 24px rgba(56,189,248,.32)!important}
  .hud-accent:active{transform:scale(.97)}
  .hud-card{cursor:pointer;transition:background .1s ease,border-color .1s ease}
  .hud-card:hover{background:rgba(255,255,255,.045)!important;border-color:rgba(255,255,255,.12)!important}
`;
...
const S = {
  ...
  mapBg:  { position:'absolute',inset:0,zIndex:0,background:`radial-gradient(ellipse at 38% 32%,#0d1828 0%,#080c14 55%,#050810 100%)`,display:'flex',alignItems:'center',justifyContent:'center',color:T.sec },
  ...
  searchBtn: { display:'flex',alignItems:'center',gap:10,padding:'9px 16px',borderRadius:12,fontFamily:FONT,fontSize:13,fontWeight:500,color:T.sec,background:'none',border:'none',minWidth:280,cursor:'pointer' },
  kbd:       { marginLeft:'auto',padding:'2px 7px',fontSize:10,fontWeight:600,fontFamily:MONO,color:T.muted,background:'rgba(255,255,255,.04)',borderRadius:5,border:`1px solid ${T.border}` },
  topCenter: {},
  topRight:  { display:'flex',alignItems:'center',gap:8 },
  viewModes: { display:'flex',gap:2,padding:3,borderRadius:12 },
  vmBtn:     { display:'flex',alignItems:'center',gap:5,padding:'7px 12px',background:'none',border:'none',borderRadius:9,fontSize:12,fontWeight:500,fontFamily:FONT,color:T.muted,whiteSpace:'nowrap',cursor:'pointer' },
  vmActive:  { color:T.text,background:'rgba(255,255,255,.09)' },
  opsBtn:    { display:'flex',alignItems:'center',gap:8,padding:'9px 14px',borderRadius:12,background:'none',border:'none',fontFamily:FONT,color:T.sec,cursor:'pointer' },
  opsBadge:  { display:'flex',alignItems:'center',gap:5,fontSize:11,fontWeight:600 },
  syncChip:  { display:'flex',alignItems:'center',gap:6,padding:'8px 12px',borderRadius:10,fontSize:10,fontWeight:500,color:T.sec,fontFamily:MONO },
  liveDot:   { width:6,height:6,borderRadius:'50%',flexShrink:0,animation:'oa-pulse 2.5s ease infinite' },

  // Bottom controls
  bottomLeft:     { position:'absolute',bottom:14,left:14,zIndex:10 },
  floorPicker:    { display:'flex',gap:2,padding:3,borderRadius:12 },
  floorBtn:       { padding:'7px 16px',background:'none',border:'none',borderRadius:9,fontSize:12,fontWeight:700,fontFamily:MONO,color:T.muted,cursor:'pointer' },
  ...
  zoomBtn:        { width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',background:'none',border:'none',fontSize:17,fontWeight:300,fontFamily:FONT,color:T.sec,cursor:'pointer' },
  fab:            { position:'absolute',bottom:14,left:'50%',transform:'translateX(-50%)',zIndex:10,display:'flex',alignItems:'center',gap:8,padding:'11px 22px',background:T.accent,color:'#080c14',border:'none',borderRadius:14,fontSize:13,fontWeight:700,fontFamily:FONT,boxShadow:'0 4px 24px rgba(56,189,248,.28)',cursor:'pointer',letterSpacing:'-.01em' },

  // Overlay
  overlay: { position:'absolute',inset:0,zIndex:50,background:'rgba(0,0,0,.48)',display:'flex',alignItems:'center',justifyContent:'center',padding:24 },

  // Browse panel
  browsePanel:      { width:'90%',maxWidth:1100,height:'85vh',borderRadius:20,display:'flex',flexDirection:'column',overflow:'hidden',boxShadow:'0 32px 100px rgba(0,0,0,.55)' },
  ...
  bpDivider:        { width:1,height:20,background:T.border,flexShrink:0 },
  ...
  bpGroupLabel:     { fontSize:10,fontWeight:700,color:T.muted,textTransform:'uppercase',letterSpacing:'.07em',marginRight:4 },
  bpCount:          { fontSize:11,color:T.muted,fontWeight:500,fontFamily:MONO },
  bpBody:           { flex:1,overflowY:'auto',padding:20 },
  bpPeopleSection:  { marginBottom:24 },
  bpSectionTitle:   { display:'flex',alignItems:'center',gap:6,fontSize:10,fontWeight:700,color:T.muted,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:10 },
  bpPeopleGrid:     { display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:6 },

  // Pills
  pill:         { padding:'4px 12px',fontSize:11,fontWeight:600,background:'rgba(255,255,255,.04)',border:`1px solid ${T.border}`,borderRadius:6,color:T.sec,fontFamily:FONT,cursor:'pointer' },
  pillActive:   { background:T.accentBg,borderColor:T.accentBorder,color:T.accent },
  pillSm:       { padding:'3px 9px',fontSize:10,fontWeight:600,background:'rgba(255,255,255,.03)',border:`1px solid ${T.border}`,borderRadius:5,color:T.muted,fontFamily:FONT,cursor:'pointer' },
  pillSmActive: { background:T.accentBg,borderColor:T.accentBorder,color:T.accent },

  // Grouped grid
  groupedGrid: { display:'flex',flexDirection:'column',gap:28 },
  group:       {},
  groupHeader: { display:'flex',alignItems:'center',gap:10,marginBottom:10 },
  groupLabel:  { fontSize:10,fontWeight:700,color:T.muted,textTransform:'uppercase',letterSpacing:'.08em',whiteSpace:'nowrap' },
  groupLine:   { flex:1,height:1,background:T.border },
  groupCount:  { fontSize:10,fontWeight:600,color:T.muted,background:'rgba(255,255,255,.04)',padding:'1px 7px',borderRadius:10,fontFamily:MONO,flexShrink:0 },
  grid:        { display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:6 },
  gridCompact: { gridTemplateColumns:'repeat(auto-fill,minmax(165px,1fr))',gap:4 },

  // Card
  card:         { position:'relative',display:'flex',flexDirection:'column',gap:6,padding:'12px 14px',background:'rgba(255,255,255,.022)',border:`1px solid ${T.border}`,borderRadius:12,textAlign:'left',fontFamily:FONT,color:T.text,overflow:'hidden' },
  cardSelected: { borderColor:T.accent,background:T.accentBg,boxShadow:`0 0 0 1px ${T.accent}28` },
  cardAccent:   { position:'absolute',left:0,top:0,bottom:0,width:2,borderRadius:'12px 0 0 12px' },
  ...
  cardLevel:    { fontSize:10,fontWeight:700,fontFamily:MONO,color:T.accent,background:T.accentBg,padding:'2px 7px',borderRadius:5,flexShrink:0 },
  ...
  // Person
  personRow:  { display:'flex',alignItems:'center',gap:10,padding:'10px 12px',background:'rgba(255,255,255,.022)',border:`1px solid ${T.border}`,borderRadius:12,fontFamily:FONT,color:T.text,textAlign:'left',cursor:'pointer' },
  personAv:   { width:32,height:32,borderRadius:'50%',background:`linear-gradient(145deg,rgba(56,189,248,.2),rgba(56,189,248,.05))`,border:`1px solid ${T.accentBorder}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:T.accent,flexShrink:0 },
  personInfo: { flex:1,display:'flex',flexDirection:'column',gap:2,minWidth:0 },

  // Route panel
  routePanel:   { width:'92%',maxWidth:1200,height:'88vh',borderRadius:20,display:'flex',flexDirection:'column',overflow:'hidden',boxShadow:'0 32px 100px rgba(0,0,0,.55)' },
  ...
  rpColDot:     { paddingTop:3,flexShrink:0 },
  ...
  rpSelectedName:  { fontSize:13,fontWeight:700 },
  rpSelectedLevel: { fontSize:10,fontWeight:700,fontFamily:MONO,color:T.accent,marginLeft:'auto' },
  rpClearBtn:   { width:20,height:20,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,.06)',border:`1px solid ${T.border}`,borderRadius:5,color:T.muted,marginLeft:4,flexShrink:0,cursor:'pointer' },
  ...
  rpSwapBtn:    { width:32,height:32,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,.05)',border:`1px solid ${T.border}`,borderRadius:10,color:T.muted,margin:'-1px',cursor:'pointer' },

  // Route result
  rpResult:    { padding:'16px 20px',borderTop:`1px solid ${T.border}`,flexShrink:0,display:'flex',gap:20,alignItems:'flex-start',flexWrap:'wrap',background:'rgba(0,0,0,.12)' },
  rrHero:      { display:'flex',alignItems:'center',gap:14,flex:'1 1 400px',flexWrap:'wrap' },
  rrIcon:      { width:42,height:42,borderRadius:12,background:T.accent,color:'#080c14',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 },
  rrMain:      { display:'flex',flexDirection:'column',gap:1 },
  rrDist:      { fontSize:22,fontWeight:800,fontFamily:MONO,letterSpacing:'-.02em' },
  rrTime:      { fontSize:11,color:T.sec },
  rrPath:      { display:'flex',alignItems:'center',gap:6,fontSize:11,color:T.sec,fontWeight:500,width:'100%',marginTop:2 },
  rrStats:     { display:'flex',gap:16,marginTop:4,width:'100%' },
  rrStat:      { display:'flex',flexDirection:'column',alignItems:'center',gap:2 },
  rrStatV:     { fontSize:14,fontWeight:800,fontFamily:MONO,lineHeight:1,display:'flex',alignItems:'center' },
  ...
  // Buttons
  accentBtn: { display:'inline-flex',alignItems:'center',gap:7,padding:'10px 18px',background:T.accent,color:'#080c14',border:'none',borderRadius:12,fontSize:13,fontWeight:700,fontFamily:FONT,cursor:'pointer' },
  ghostBtn:  { display:'inline-flex',alignItems:'center',justifyContent:'center',gap:6,padding:'9px 14px',background:'none',color:T.sec,border:`1px solid ${T.border}`,borderRadius:12,fontSize:12,fontWeight:600,fontFamily:FONT,cursor:'pointer' },
  iconBtn:   { width:28,height:28,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,.04)',border:`1px solid ${T.border}`,borderRadius:8,color:T.sec,flexShrink:0,cursor:'pointer' },

  // Detail float
  detailFloat: { position:'absolute',top:68,right:14,zIndex:20,width:308,borderRadius:18,padding:18,display:'flex',flexDirection:'column',gap:14,boxShadow:'0 20px 70px rgba(0,0,0,.4)',maxHeight:'calc(100vh - 90px)',overflowY:'auto' },
  statusPill:  { display:'inline-flex',alignItems:'center',gap:5,padding:'4px 10px',fontSize:11,fontWeight:600,borderRadius:20 },

  // Ops
  opsPanel: { position:'absolute',top:60,right:14,zIndex:40,width:340,borderRadius:18,boxShadow:'0 20px 70px rgba(0,0,0,.45)',maxHeight:'calc(100vh - 80px)',overflow:'hidden',display:'flex',flexDirection:'column' },

  // Checkbox
  checkRow:   { display:'flex',alignItems:'center',gap:7,cursor:'pointer',userSelect:'none',fontSize:12,color:T.sec,fontWeight:500 },
  checkBox:   { width:15,height:15,borderRadius:4,border:`1.5px solid rgba(255,255,255,.16)`,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'all .12s',color:'#080c14',flexShrink:0 },
